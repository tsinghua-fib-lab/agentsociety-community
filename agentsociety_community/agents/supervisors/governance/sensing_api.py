...
class SensingAPI:
    ...
    def _set_governance(self, governance: "GovernanceBase"):
        self._governance = governance

    @property
    def governance(self) -> "GovernanceBase":
        assert self._governance is not None, "Governance not set"
    ...
    def get_all_historical_posts(self) -> list[dict[str, Any]]:
        ...
        return [p.copy() for p in self.governance.global_posts_history]
    ...
    def get_posts_last_k_rounds(self, k: int) -> list[dict[str, Any]]:
        ...
        if k <= 0:
            return []
        current_round = (
            self.governance.get_current_round_number()
        )  # 需要 Simulation 实现此方法
    ...
    def get_post_content_by_id(self, post_id: str) -> Optional[str]:
        ...
        for post in self.governance.current_round_posts_buffer:
            if post["post_id"] == post_id:
                return post["content"]
        for post in self.governance.global_posts_history:
            if post["post_id"] == post_id:
                return post["content"]
        return None
    ...
    def get_posts_containing_keywords(
        ...
        current_sim_round = self.governance.get_current_round_number()
        effective_last_k = (
    ...
    def get_agent_intervention_history(self, agent_id: int) -> list[dict[str, Any]]:
        ...
        agent = self.governance.agent_map.get(agent_id)
        if agent:
            return [
    ...
    def get_public_network_structure(self) -> dict[str, list[Any]]:
        ...
        return self.governance.network.get_network_structure()
    ...
    def get_public_node_degree(self, agent_id: int) -> Tuple[int, int]:
        ...
        in_degree = len(self.governance.network.followers(agent_id))
        # 出度：agent_id 关注了多少人 -> agent_id 是 source
        out_degree = len(self.governance.network.following(agent_id))
    ...
    def get_top_degree_nodes(self, top_k: int, degree_type: str = "total") -> list[int]:
        ...
        if top_k <= 0:
            return []
        degrees_map = {}
        for node_id in self.governance.agent_map.keys():  # 遍历所有注册的智能体
    ...
    def get_public_neighbors(self, agent_id: int) -> list[int]:
        ...
        if agent_id not in self.governance.agent_map:
            return []
        followers = self.governance.network.followers(agent_id)
        following = self.governance.network.following(agent_id)
    ...
    def get_remaining_intervention_quotas(self) -> dict[str, dict[str, int]]:
        ...
        for type, limits in self.intervention_quotas.items():
            used_stats = self.governance.intervention_stats.get(
    ...
    def get_globally_blocked_elements(self) -> dict[str, list[Any]]:
        ...
        return {
            "blocked_agents": sorted(list(self.governance.banned_agent_ids)),
            "cut_edges": sorted(
                [list(edge) for edge in self.governance.globally_removed_edges]
            ),
        }
    ...
    def get_interventions_by_me_this_round(self) -> list[dict[str, Any]]:
        ...
        return [
            intervention.copy()
            for intervention in self.governance.current_round_interventions
        ]
    ...
    def get_interventions_by_me(self) -> list[dict[str, Any]]:
        ...
        return [
            intervention.copy()
            for intervention in self.governance.all_historical_interventions_log
        ]

    # --- 平台与比赛信息类函数 (29-31) --- #
    def get_current_round_number(self) -> int:
        """获取当前的比赛轮次。"""
        return self.governance.get_current_round_number()

    def get_total_simulation_rounds(self) -> int:
        """获取本次模拟比赛总共的轮次数。"""
        return self.governance.get_total_simulation_rounds()

    def get_rumor_topic_description(self) -> list[str]:
        """获取当前比赛设定的核心谣言主题的文字描述（问卷用）。"""
        return self.governance.rumor_topic_description
